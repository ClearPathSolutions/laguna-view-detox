...
**Partially done:** **T-48** — the route is now **fail-safe**: with `LEAD_WEBHOOK_URL` unset it logs CRITICAL, keeps the server-side record, and returns 503 with "please call us at (866) 932-3206" instead of falsely confirming a callback. Verified all three paths (no webhook → 503; working webhook → `{ok:true}` with the lead delivered; honeypot → silently dropped). **Setting the env var is still required to restore online submissions.** **T-21** — component, placements and GMB CTA shipped; `lib/reviews.ts` is deliberately empty pending real Google review data, and no rating schema is emitted until it holds real reviews. **T-47** — hero, CTA band and logo done; who-we-treat/locations/blog imagery blocked on the brand decision. **T-04** — code complete; needs `NEXT_PUBLIC_GA_ID`.
